define(["CellEvolutionStrategy", "Cell"], function (CellEvolutionStrategy, Cell) {
    describe("Getting new state of cell that comes from some area", function () {

        var cellEvolutionStrategy = new CellEvolutionStrategy();
        ...
        var expectStrategyBehavior = function (
            ...
            var cell = new Cell(givenCellIsAlive);
            var neighbors = [];
            for (var i = 0; i < numberOfAliveNeighbors; i++) {
                neighbors.push(new Cell(true));
            }

            for (var i = 0; i < numberOfDeadNeighbors; i++) {
                neighbors.push(new Cell(false));
            }
